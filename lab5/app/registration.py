...
import subprocess
import shlex
from code_executor import CodeExecutionForm, execute_code_safely
...
def execute_code():
    ...
    Принимает POST запрос с полями code и timeout
    """
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        output, error = execute_code_safely(code, timeout)
        
        if error:
            return render_template('execute.html', form=form, error=error, output=None)
        else:
            return render_template('execute.html', form=form, output=output, error=None)
    
    return render_template('execute.html', form=form, output=None, error=None)
